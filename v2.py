...
class MultiHeadAttention(nn.Module): 
    ...
    def __init__(self, num_heads, head_size): 
        ...
        #self.proj = nn.Linear(head_size * num_heads, head_size) # project the concatenated outputs from all heads to the original size
        #self.dropout = nn.Dropout(dropout)

    def forward(self, x): 
        return torch.cat([head(x) for head in self.heads], dim=-1) # concatenate the outputs from all heads along the last dimension
...
class BigramLanguageModel(nn.Module):
    def __init__(self):
        ...
        self.sa_head = MultiHeadAttention(4, n_embd//4) # multi-head attention head with 4 heads and each head has n_embd//4 (8) dimensions
    ...
    def forward(self, idx, targets=None):
        ...
        logits = self.lm_head(x) # compute language model logits. (B,T,vocab_size) 
